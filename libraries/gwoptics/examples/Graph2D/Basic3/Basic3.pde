 ...
 *  It plots two functions, and shows some more methods available to alter the graph
 **/
...
/**
 *  Equations that are to plot must be encapsulated in a 
...
void setup()
{
  ...
  // Graph2D object, arguments are parent object, xsize, ysize, cross axes at zero pt
  g = new Graph2D(this, 400, 200, true); 

  // setting attributes for the X and Y-Axes
  g.setYAxisMin(0.1);
  g.setYAxisMax(10000);
  g.setXAxisMin(0.001);
  g.setXAxisMax(10);
  ...
  // switching off the border, and changing the label positions
  g.setNoBorder(); 
  ...
  g.setBackground(gb);
  // offset of graph top left corner from the app window corner
  g.position.y = 50;
  g.position.x = 60;
