/* 
 Serial String Reader 5 -- for Win32 export, use Serial.list[1], or portName="COM_" prn ?
 C:\\path for data save. With energia on PC this still works, COM11 is second on list, i.e. [1]
...
// pared import list to just what's needed here
...
void setup() 
{
  ...
  String portName = Serial.list()[4];  // usually 0 for Mac, 1 for PC, 4 for iMac
...
{   
  ...
  // you can access vals by iterating over rows in table where they are stored
  if ( volTab.getRowCount() > 1)   // may not need this condx, doesn't show 1st set
  {
  ...
  noLoop(); // just draw vals once each time called on redraw
}  // end draw
...
{                       // presume it runs in event thread
  ...
  //Win: 
  if (key == 's') { 
    saveTable(volTab, "C:\\Users\\alexM\\Documents\\Processing\\Sketchbook\\SerialStringReader5\\data\\newV.csv"); 
    exit();
  }
