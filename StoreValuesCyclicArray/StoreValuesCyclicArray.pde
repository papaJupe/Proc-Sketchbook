/** StoreValuesCyclicArray
 *orig: Proc/Help/Basic/Input/Storing Input 
 * Uses: frameRate,%modulo,cyclic write to update array
 * Move the mouse across the screen to change the position
 * of the circle. The x,y locs of the mouse are put
 * into an array and played back every frame. Between each
 * frame, the newest values are added into the x,y arrays, cyclically,
 * and the oldest (at that index) value is deleted. Draw() displays the array
 * continuously, oldest data (newest index +1) drawing smallest circle
 */
  ...
  for (int i = 0; i < num; i++) // 0->59, must complete for loop to advance
    { // indx [which] is latest to get overwritten above, so
      ...
      int index = (which+1 + i) % num;  //this cycles 0->59 also, each
       ...
       // completes, the frame can advance giving a new frameCt and which index
