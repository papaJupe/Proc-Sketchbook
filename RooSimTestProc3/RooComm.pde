/* RooCommA 0.97 adapted from RoombaComm v 0.96 from roombahacking.com
 book site
 *  revise AM 1804 to be instantiable class for comm to Roomba
 ...
 class, calls its members for comm. [__TestProc2 sent bytes itself]
...
{  // declare, init misc. variables, constants
  ...
  static public final String VERSION = "0.97";
  ...
  /** computed boolean if Roomba is 'errored out' of safe mode 
   -- but I can't see if running mode is actually changed */
  ...
  byte[] sensor_bytes = new byte[1024];  // array size, need 1024 ?
  ...
  // 3 overloaded constructors, +/- autoUpdate
    ...
    connected = false;
    mode = MODE_UNKNOWN;
    // connect() port test called from sketch after obj exists
  }
   ...
   */
  public boolean connect()  // return T/F, would prefer to test
      // actual connection to R, not just if there's a port obj
  {
    if (myPort != null)
      connected = true; 
    else
      connected = false; // could exit here vs. going back to sketch
    // println("connection to port failed");
    return connected;
  }  // end connect
  ...
  public void reset() {
    ...
    pause(300);
    //  test if this work
    if (updateSensors()) println("OK, we're back on");
  }
...
/* ========  sensor methods, vars, constants, serialEvent ========== */
  ...
  }  // end sensors() w/ datapacket

  // if called w/o param, gets all sensor data

  public void sensors() {
    // readRequestLength = 26;  set above
    sensors( SENSORS_ALL );  // should be 0, all sensors
  }  // end sensors() w/ no params

     // called by good serialEvent, could go there instead
     // but keep separate to do stuff after good input
  public void computeSensors() {
    sensorsValid = true;  // ? need, only called if true
    sensorsLastUpdateTime = System.currentTimeMillis();
    computeSafetyFault();  // supposed to set mode to Passive if
       // any fault, but does setting this do anything ?
  }
  ...
  public boolean safetyFault() { 
    return safetyFault;
     ...
     // this secondary validation fx is not called anywhere
  public boolean sensorsValid() {
    // FIXME: am I broken ? sensVali var set true in computeSensors
    // sV var may be valid but stale, so how would this be used?
    if ( sensorsValid ) { 
  ...
  public String sensorsAsString() {
    String sd="";
    if ( debug ) { // fill the sd array with sensor hex data
      sd = "\n";
      for ( int i=0; i<26; i++ )
    ...
    "bump:" + 
      ...
      " chrg:" + charge() +
      " capa:" + capacity() +
      sd;  // if debug on, this is hex of above?, else '\n'
  }
