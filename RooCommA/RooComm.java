/* RooCommA adapted from
 *  RoombaComm base from v 0.96 from roombahacking.com book site
 *  recast by AM 1804 to be instantiable class for comm to Roomba
    -- used by Proc code which has user I/O code, not usable as is
    here.
 ...
 * Thus, this object's form may change in the future.
 
 * AM: I could perhaps leave this as is and make a subclass out of
   RooCommSer that used Serial instead of rxtx lib ? (flaky at best)
   but first will try editing this to make it do comm w/o the subclass
 ...
 *   roomba.disconnect();
 * </pre>
 * to do: test wakeup DTR method from RCS
      disable unused methods, enable used methods w/ code from RCS
...
public class RooComm    // was abstract
{
    /** version of the library */
    static public final String VERSION = "0.97"; // adapted from 0.96

    /** turns on/off various debugging messages */
    public boolean debug = false;
    
    /** mm distance between classic roomba wheels 
         was int probably should be float */
    public static final float wheelbase = 258;
     
    /** mm/deg is circumference distance divided by 360 degrees; 
    in SPIN, rotating around center, the distance each wheel moves
   (one +, one -) per deg. in mm is d * pi/ 360, and d = 258 in spin 
   Doubt if same applies to other movement */
    ...
    */
    ...
    /** current mode, if known, public so .pde can get easily */
    public int mode;

    /** current speed for movement operations that don't take a speed */
    public int speed = defaultSpeed;
    
    // index of good port in Serial.list sent to this class
    private int portIndex;
    // string name of port that comes from Ser.list
    private String portName;
    
    
    /** computed boolean for when Roomba is 'errored out' of safe mode 
    -- but so far I can't see if mode is actually changed */
    ...
    byte[] sensor_bytes = new byte[1024];  // why this huge array size?
  
    /** ? connected to a serial port (local), maybe not to roomba */
    boolean connected = false;
    
   // 3 overloaded constructors, all get a portID, can autoUpdate

    public RooComm(int prtidx) {
        connected = false;
        mode = MODE_UNKNOWN;
        portName = Serial.list()[prtidx]; 
  
  // open serial port: the connection to BT dongle, not R
  // dongle's baud has to be set to R's speed, which is 57600
  Serial btPort = new Serial(this, portName, 57600); // speed of R serial
  }
  
  public RooComm(int prtidx, boolean autoUpdate)
        this();  // does this load the stuff above?
        if( autoUpdate )
            startAutoUpdate();
    }

    public RooComm(int prtidx, boolean autoUpdate, int updateTime) {
        this(autoUpdate);
    ...
    /**
     * List available ports (to implement in sub class)
     * @return a list of available portids, if applicable
   ...
   // public abstract  void disconnect();
    ...
    public boolean send(int b)
      {   //adapt from RCS
              try {
            btPort.write(b & 0xff);  // for good measure do the &
     ...
     */
    public void wakeup() {
        logmsg(" wake subclass not implemented"); // not sure it can be
    }
    /* Power off the Roomba. Once off, the only way to wake it
     * is by wakeup() (not yet, maybe impossible) or 
     by physically pressing the Power button
     * @see wakeup() v.supra
     */
     ...
     * Safe mode is the preferred working state
     * when playing with the Roomba as it provides some measure of 
     * autonomous safety if it encounters a cliff or is picked up
    ...
    public void reset() {
        ...
        startup();
        control();
        updateSensors();  // to do: implement this
    }
    ...
    public void control() { 
        logmsg("control");
        mode = MODE_SAFE; // probably not yet
        send( CONTROL );
 ...
 /** Query Roomba sensor status and sync its state with this object's.
 * Subclasses should query Roomba and fill up 'sensor_bytes' with the full
 * sensor data set
 * If a RoombaComm object is constructed with 'autoUpdate' true, 
 * calling this method is as needed, because a separate thread is created
 * for sensor updating calling the sensors() fx
 * e.g. reset() calls it below
 * @return true on 'valid' sensor update, false otherwise
 */
 ...
 */
  public void sensors(int packetcode ) {
        ...
        switch (packetcode) {
        case 0: readRequestLength = 26; break; // all
        case 1: readRequestLength = 10; break;  // physical
        case 2: readRequestLength = 6; break;   // internal
        case 3: readRequestLength = 10; break;  // power related
    ...
    /* get all sensor data, if called w/o param
     */
    public void sensors() {
     ...
     * @param sensorList -- byte array naming the sensor groups to get
     * @param returnLen -- number of bytes of data expected to get
     */
    public void queryList(byte[] sensorList, int returnLen)
    {
    	...
    	cmd[i++] = (byte) QUERYLIST;  // should just fill cmd[0]
    	for (i=0; i<sensorList.length; i++) // fills index 1 up
     ...
     * Sends drive(0,0) --  v.i. <-- lower level method
     */
    public void stop() {
        logmsg("stop");
        drive( 0, 0 );
    ...
    /** Get speed for movement cmds -- speed is public already, so ? */
  ...
  // does this just reset mode var or actually change bot's mode ?  where ?
        return safetyFault;
    }

    /* mode fx() Returns current connected state.  
...
// amount of dirt seen by left dirt sensor 
    ...
    //
    // utility method, conversion, alias, shortcut
    //
